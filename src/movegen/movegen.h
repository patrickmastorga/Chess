...
namespace movegen {
    ...
    bool make_move(Board &board, Move &move, bool unsafe = false) noexcept;
    ...
    bool castling_move_is_legal(const Board &board, Move &move, bool unsafe = false) noexcept;

    // return true if inputted move is pseudo legal in the given board
    bool is_pseudo_legal(const Board &board, Move &move) noexcept;
